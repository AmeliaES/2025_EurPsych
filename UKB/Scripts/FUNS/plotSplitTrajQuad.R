...
# also pass dataset, condition eg. IL6_tertile, and a vector of colours where the length is the same as the length of condition levels
# ------------------------------------

plotSplitTrajQuad <- function(model, data, condition,conditionPlot, plotColours, legendTitle, legendLabels, sex = FALSE){
  ...
  CI_glht <- lapply(ageCalcs, function(x){
    ...
    rowname <- paste0("PHQ-2 Score")
    res <- data.frame(estimate = res$Estimate,
                      conf.low = res$`2.5 %`,
                      conf.high = res$`97.5 %`) %>%
      dplyr::mutate(condition = levelNames)
