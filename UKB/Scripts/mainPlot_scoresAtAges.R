...
files <- str_subset(list.files("Output/traj_inflam/tableScoresAtAges", full.names = T), "difference") %>%
  str_subset(., "Sens", negate = T)

mainFig <- lapply(files, function(file){
  ...
  longData <- lapply(c("Difference", "Lower_CI", "Upper_CI", "uncorrected_p", "FDR_p"), function(col){
    res %>%
...
longData <- longData %>%
  mutate(sig = as.factor(
    case_when(FDR_p == "p<0.0001" | as.numeric(FDR_p) <= 0.05 ~ "pFDR \U2264 0.05",
              (FDR_p != "p<0.0001" | as.numeric(FDR_p) > 0.05) & as.numeric(uncorrected_p) < 0.05 ~ "p (uncorrected) \U2264 0.05",
